/*
 * Taken from https://www.glfw.org/docs/latest/quick.html.
 * Modified to use GLM instead of "linmath.h".
 */
...
int main(void) {
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        mat4 m = mat4(1.0f); // Identity matrix for model
        mat4 p;              // Projection matrix
        mat4 mvp;            // Model-View-Projection matrix

        // Set up rotation around Z axis
        m = rotate(m, (float)glfwGetTime(), vec3(0.0f, 0.0f, 1.0f));

        // Set up orthographic projection
        p = ortho(-ratio, ratio, -1.0f, 1.0f, 1.0f, -1.0f);

        // Multiply projection and model matrices to get MVP
        mvp = p * m;
